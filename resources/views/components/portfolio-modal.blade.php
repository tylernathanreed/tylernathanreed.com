        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="portfolio-modal-heading">
                    <div class="portfolio-modal-icon mr-4{{ !empty($icon) ? " {$icon} text-white p-2" : '' }} bg-{{ $color }}">
                        @if(is_null($icon))
                            <img class="img-fluid" src="{{ url("images/ecosystem/{$key}.min.svg") }}" alt="{{ $name }}" />
                        @endif
                    </div>
                    <h2 class="portfolio-modal-title text-secondary mb-0" id="portfolio-modal-{{ $key }}-label">{{ $name }}</h2>
                </div>
