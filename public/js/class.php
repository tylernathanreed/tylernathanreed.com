...
class stream_get extends getinfo {
	...
	function get($url){
		...
		// $linkdown = 'http://'.$sv_name.$hosting.'/'.$job['hash'].'/'.urlencode($filename); # If Active Generates Hosts Links
        $linkdown = $link; # If Active Then Generates Direct Links
		...
		if(empty($this->zlink)==false && empty($this->link1_zip)==false && empty($link)==false ){ 
			$datalink = $this->curl($this->link1_zip.$lik,'','',0);
			if (preg_match('%(http:\/\/.++)%U', $datalink, $shortlink)) $lik = trim($shortlink[1]);
			else $lik = $linkdown;	
		}
		############ End short link  ##########
