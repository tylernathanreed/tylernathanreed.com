...
export default class Spa {
  ...
  /**
   * Register a callback to be called before Spa starts. This is used to bootstrap
   * addons, tools, custom fields, or anything else Spa needs
   */
  booting(callback) {
  ...
  /**
   * Start the Spa app by calling each of the tool's callbacks and then creating
  ...
  /**
   * Return an axios instance configured to make requests to Spa's API
  ...
  /**
   * Register a listener on Spa's built-in event bus
   */
  $on(...args) {
    this.bus.$on(...args)
  }
  ...
  /**
   * Determine if Spa is missing the requested resource with the given uri key
   */
